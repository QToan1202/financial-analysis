...
class MongoDBSaver(BaseCheckpointSaver):
    ...
    @classmethod
    @contextmanager
    def from_conn_info(cls, *, host: str, port: Optional[int] = None, db_name: str) -> Iterator["MongoDBSaver"]:
        client = None
        try:
            client = MongoClient(host=host, port=port)
            yield MongoDBSaver(client=client, db_name=db_name)
        finally:
            if client:
                client.close()
    ...
    def get_tuple(self, config: RunnableConfig) -> Optional[CheckpointTuple]:
        ...
        if checkpoint_id := get_checkpoint_id(config=config):
            query = {
                "thread_id": thread_id,
        ...
        result = self.db["checkpoints"].find(query).sort(
            "checkpoint_id", DESCENDING).limit(1)
        for doc in result:
            config_values = {
                "thread_id": thread_id,
                "checkpoint_ns": checkpoint_ns,
            ...
            serialized_writes = self.db["checkpoint_writes"].find(
                config_values)
            pending_writes = [
                (
                    doc["task_id"],
    ...
    def list(
        ...
        *,
        ...
        result = self.db["checkpoints"].find(
